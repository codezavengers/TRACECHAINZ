...
export function AppShell({
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#0d1017] text-slate-100 font-sans selection:bg-amber-400 selection:text-black">
      {/* Sidebar for Desktop */}
      <aside className="hidden lg:flex w-64 flex-col border-r border-white/10 bg-[#12151e] shrink-0">
        {/* Brand / Logo */}
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div className="flex items-center gap-2.5">
            <div className="size-9 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-black font-black text-lg shadow-md shadow-amber-400/20">
              TC
            </div>
            <div>
        ...
        <div className="p-3.5">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-amber-400 hover:bg-amber-300 px-3.5 py-2.5 text-xs font-bold text-black transition shadow-lg shadow-amber-400/20"
          >
        ...
        <div className="p-3.5 border-t border-white/10 bg-black/30">
          <div className="flex items-center justify-between text-[11px] text-slate-400">
            ...
            <span className="font-mono text-amber-400">6 Chains</span>
          </div>
        </div>
      </aside>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        {/* Top Header */}
        <header className="flex h-16 items-center justify-between border-b border-white/10 bg-[#12151e] px-4 sm:px-6 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <span className="rounded-full bg-emerald-500/10 border border-emerald-500/30 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-400 flex items-center gap-1.5">
              ...
              {btcData && (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-full border border-amber-400/30 bg-amber-400/10 hover:bg-amber-400/20 px-2.5 py-0.5 text-[11px] font-mono text-amber-300 transition"
                >
                  <span className={`size-1.5 rounded-full bg-amber-400 ${btcLoading ? "animate-ping" : "animate-pulse"}`} />
                  <span className="font-bold">BTC ${btcData.priceUsd.toLocaleString()}</span>
          ...
          <div className="flex items-center gap-3 relative">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-black/40 hover:bg-white/5 p-1.5 pr-3 transition text-left"
              >
                <div className="size-7 rounded-lg bg-amber-400/20 border border-amber-400/30 flex items-center justify-center text-amber-300 font-bold text-xs">
              ...
              {/* User Dropdown */}
              {userDropdownOpen && (
                <div className="absolute right-0 top-12 w-64 rounded-xl border border-white/10 bg-[#161a24] p-2 shadow-2xl z-50 animate-in fade-in space-y-1">
                  ...
                  {availableUsers.map((u) => (
                    <button
                      ...
                      className={`w-full text-left p-2 rounded-lg text-xs transition flex items-center justify-between ${
                        u.id === currentUser.id ? "bg-amber-400/20 text-amber-300 font-semibold" : "text-slate-300 hover:bg-white/5"
                      }`}
                    >
                      <div>
                  ...
                  <div className="pt-1 border-t border-white/10">
                    <button
                      ...
                      className="w-full text-left p-2 rounded-lg text-xs text-amber-400 hover:bg-white/5 flex items-center gap-1.5"
                    >
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-[#0e1119]">
