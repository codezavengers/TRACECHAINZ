...
export function CreateCaseModal({ isOpen, onClose, onCreateCase }: CreateCaseModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-2xl rounded-2xl border border-white/10 bg-[#161a24] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4 bg-[#181c26]">
          <div className="flex items-center gap-2.5">
            <FilePlus2 className="size-5 text-amber-400" />
            <h2 className="text-base font-bold text-white">Open New Fraud Investigation</h2>
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white rounded-lg p-1 hover:bg-white/10"
          >
            <X className="size-4" />
          </button>
        </div>
        ...
        <div className="flex border-b border-white/10 bg-black/30 px-5 pt-3">
          ...
          >
            ...
            SIH Presentation Command Center (6 Scenarios)
          </button>
        </div>
