...
export class TronProvider extends AbstractProvider {
  ...
  async getLatestBlock(): Promise<number | null> {
    try {
      const res = await fetchJson<{ data?: Array<{ block_header?: { raw_data?: { number?: number } } }> }>(
        this.url("/v1/blocks/latest"),
        { label: "tron latest block", headers: this.headers(), retries: 1 },
      )
      return res.data?.[0]?.block_header?.raw_data?.number ?? null
    } catch {
      return null
    }
  }
  ...
  private nativeTxToTransaction(address: string, tx: TronTx): Transaction | null {
    ...
    return normalizeNative({
      ...
      // TronGrid is TRON's official native node infrastructure REST API.
